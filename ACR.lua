...
function ACR:updateOutput(input)
  ...
  for output_x = 1, self.output:size()[2] do
    for output_y = 1, self.output:size()[3] do
      ...
      self.output[{{}, output_x, output_y}] = torch.cmul(intensity, getInterpolatedTemplateValue(
                                                  template,
                                                  template_coords[{{},1}], -- x
                                                  template_coords[{{},2}])) -- y
...
function ACR:updateGradInput(input, gradOutput)
  ...
  for output_x = 1, self.output:size()[2] do
    for output_y = 1, self.output:size()[3] do
      ...
      local x_high_coeff = torch.zeros(3)
      ...
      local y_high_coeff = torch.zeros(3)
      ...
      self.gradPose[{{},1,1}] = self.gradPose[{{},1,1}] + torch.cmul(gradOutput[{{},output_x,output_y}], torch.cmul( (getTemplateValue(template, x_high, y_high)*output_x - getTemplateValue(template, x_low, y_high)*output_x), (pose[{{},2,3}] - y_low + pose[{{},2,1}]*output_x + pose[{{},2,2}]*output_y))) - torch.cmul((getTemplateValue(template, x_high, y_low)*output_x - getTemplateValue(template, x_low, y_low)*output_x), (pose[{{},2,3}] - y_high + pose[{{},2,1}]*output_x + pose[{{},2,2}]*output_y))
      
      self.gradPose[{{},1,2}] = self.gradPose[{{},1,2}] + torch.cmul(gradOutput[{{},output_x,output_y}], torch.cmul((getTemplateValue(template, x_high, y_high)*output_y - getTemplateValue(template, x_low, y_high)*output_y), (pose[{{},2,3}] - y_low + pose[{{},2,1}]*output_x + pose[{{},2,2}]*output_y))) - torch.cmul((getTemplateValue(template, x_high, y_low)*output_y - getTemplateValue(template, x_low, y_low)*output_y),(pose[{{},2,3}] - y_high + pose[{{},2,1}]*output_x + pose[{{},2,2}]*output_y))
        
      self.gradPose[{{},1,3}] = self.gradPose[{{},1,3}] + torch.cmul(gradOutput[{{},output_x,output_y}], torch.cmul( (getTemplateValue(template, x_high, y_high) - getTemplateValue(template, x_low, y_high)),(pose[{{},2,3}] - y_low + pose[{{},2,1}]*output_x + pose[{{},2,2}]*output_y))) - torch.cmul((getTemplateValue(template, x_high, y_low) - getTemplateValue(template, x_low, y_low)),(pose[{{},2,3}] - y_high + pose[{{},2,1}]*output_x + pose[{{},2,2}]*output_y))
     
       
      self.gradPose[{{},2,1}] = self.gradPose[{{},2,1}] +  torch.cmul(gradOutput[{{},output_x,output_y}], torch.cmul( getTemplateValue(template, x_high, y_high), (pose[{{},1,3}] - x_low + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y)) - torch.cmul(getTemplateValue(template, x_low, y_high), (pose[{{},1,3}] - x_high + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y)))*output_x - (  torch.cmul(getTemplateValue(template, x_high, y_low),(pose[{{},1,3}] - x_low + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y)) - torch.cmul(getTemplateValue(template, x_low, y_low),(pose[{{},1,3}] - x_high + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y)))*output_x
      

      self.gradPose[{{},2,2}] = self.gradPose[{{},2,2}] + torch.cmul(gradOutput[{{},output_x,output_y}],(torch.cmul( getTemplateValue(template, x_high, y_high),(pose[1][3] - x_low + pose[1][1]*output_x + pose[1][2]*output_y)) - torch.cmul(getTemplateValue(template, x_low, y_high),(pose[1][3] - x_high + pose[1][1]*output_x + pose[1][2]*output_y))))*output_y - ( torch.cmul(getTemplateValue(template, x_high, y_low),(pose[1][3] - x_low + pose[1][1]*output_x + pose[1][2]*output_y)) - torch.cmul(getTemplateValue(template, x_low, y_low),(pose[1][3] - x_high + pose[1][1]*output_x + pose[1][2]*output_y)))*output_y
      
      self.gradPose[{{},2,3}] = self.gradPose[{{},2,3}] + torch.cmul(gradOutput[{{},output_x,output_y}], torch.cmul( getTemplateValue(template, x_high, y_high), (pose[{{},1,3}] - x_low + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y))) - torch.cmul( getTemplateValue(template, x_low, y_high),(pose[{{},1,3}] - x_high + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y)) - torch.cmul(getTemplateValue(template, x_high, y_low), (pose[{{},1,3}] - x_low + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y)) + torch.cmul(getTemplateValue(template, x_low, y_low), (pose[{{},1,3}] - x_high + pose[{{},1,1}]*output_x + pose[{{},1,2}]*output_y))
...
function getInterpolatedTemplateValue(template, template_x, template_y)
  ...
  return torch.cmul(getTemplateValue(template, x_low,  y_low) , torch.cmul(x_low_coeff  , y_low_coeff))  +
         torch.cmul(getTemplateValue(template, x_high, y_low) , torch.cmul(x_high_coeff , y_low_coeff )) +
         torch.cmul(getTemplateValue(template, x_low,  y_high), torch.cmul(x_low_coeff  , y_high_coeff ))+
         torch.cmul(getTemplateValue(template, x_high, y_high), torch.cmul(x_high_coeff , y_high_coeff ))
end
